...
class Vide_pad(object):
    ...
    def __call__(self, x):
        ...
        if tt>self.t:
            t0 = (tt-self.t)//2
            return x[t0:t0+self.t]
        else:
            p1 = (self.t - tt)//2
            p2 = self.t - tt -p1
            x = torch.nn.functional.pad(x, pad=(0, 0, 0, 0, p1, p2), mode='constant', value=0)
        return x

class Vide_random_pad(object):
    def __init__(self, t=32):
        self.t = t
    ...
    def __call__(self, x):
        tt = x.size(0)
        if tt>self.t:
            t0 = random.randint(0,tt-self.t)
            return x[t0:t0+self.t]
        else:
            p1 = random.randint(0,self.t-tt)
            p2 = self.t - tt -p1
            x = torch.nn.functional.pad(x, pad=(0, 0, 0, 0, p1, p2), mode='constant', value=0)
        return x
